...
import { ResumenEjecutivo } from "@/components/modelo-ingresos/resumen-ejecutivo";
...
export default function ModeloIngresosPage() {
  ...
  const resultadoAnual = useMemo(() => {
    const ingresoBrutoAnual = monthlyResults.reduce(
      (s, r) => s + r.ingresoBruto,
      0
    );
    const comisionesAnuales = monthlyResults.reduce(
      (s, r) => s + r.comisiones,
      0
    );
    const ivaNetoAnual = monthlyResults.reduce(
      (s, r) => s + r.iva.ivaNeto,
      0
    );
    const gastosOpAnuales = totalGastosOp * 12;
    const utilidadGravable = monthlyResults.reduce(
